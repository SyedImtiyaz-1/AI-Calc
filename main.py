...
if run:
    try:
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
            raise Exception("Unable to access the camera. Please check your camera settings and permissions.")
        ...
        while run:
            ...
            if info:
                draw(info)
                output_text = sendToAI(model, st.session_state.canvas, info[0])
                if output_text:
                    output_text_area.text(output_text)

            combined_frame = cv2.addWeighted(frame, 0.7, st.session_state.canvas, 0.3, 0)
            FRAME_WINDOW.image(cv2.cvtColor(combined_frame, cv2.COLOR_BGR2RGB))
        ...
        st.info("If you're running this app in a deployed environment, camera access might be restricted. "
                "Please try running the app locally or contact the administrator for assistance.")

else:
    st.write("Camera is deactivated. Check the 'Activate Camera' box to start.")